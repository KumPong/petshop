...
* **ความสนใจ:** การจัดระเบียบสินค้า, การบริการลูกค้า, ความรวดเร็วในการทำงาน
...
```mermaid
    ...
    %% เฟส 5: ติดตามพัสดุ และยืนยันรับสินค้า (Tracking)
    rect
        Customer->>UI: เปิดหน้าติดตามพัสดุ /tracking/:orderId
        UI->>API: GET /api/orders/:id
        API->>DB: อ่านออเดอร์จาก order.json
        DB-->>API: คืนค่าออเดอร์
        API-->>UI: คืนออเดอร์ + timeline สถานะ (สร้างสดจาก statusHistory)
        UI-->>Customer: แสดงไทม์ไลน์ + เลขพัสดุ

        opt สถานะเป็น "Shipped" (พัสดุถึงมือแล้ว)
            Customer->>UI: กดปุ่ม "ยืนยันรับสินค้า" แล้วยืนยันใน dialog
            UI->>API: PATCH /api/orders/:id/status {status: 'Delivered'}
            API->>API: ตรวจสิทธิ์ — ต้องเป็นเจ้าของออเดอร์ และสถานะเดิมเป็น Shipped เท่านั้น
            API->>DB: อัปเดต status + statusHistory ใน order.json
            DB-->>API: บันทึกสำเร็จ
            API-->>UI: คืนค่าออเดอร์ (Delivered)
            UI-->>Customer: แสดง Popup "ยืนยันการรับสินค้าเรียบร้อยแล้ว" + อัปเดตไทม์ไลน์
        end
    end
```
2.Staff
```mermaid
    ...
    rect
        ...
        loop เดินสถานะทีละขั้น (Confirmed → Processing → Packed → Shipped)
        ...
        Note over Staff,DB: Staff จบหน้าที่ที่สถานะ "Shipped" — ขั้นสุดท้าย (Shipped → Delivered)<br/>ลูกค้าเป็นผู้กดยืนยันรับสินค้าเองจากหน้า Tracking (ดู Sequence "1.Customer" เฟส 5)<br/>หน้าจอ Staff จะขึ้นว่า "รอลูกค้ายืนยันรับสินค้า" แต่ยังกด "พบปัญหา" ได้
    end
```
...
| :--- | :--- |
| **Manager** | แดชบอร์ด, จัดการผู้ใช้, รายงานสินค้าคงเหลือ, รายงานยอดขาย, จัดการสินค้า, รีสต็อก |
| **Staff** | ดูรายการคำสั่งซื้อ, ตรวจสอบและยืนยันคำสั่งซื้อ, จัดเตรียมสินค้า, อัปเดตสถานะ, เช็คสต็อกสินค้า |
| **Customer** | สมัครสมาชิก, เข้าสู่ระบบ, ค้นหาสินค้า, เพิ่มสินค้าลงตะกร้า, สั่งซื้อสินค้า, ชำระเงิน, ติดตามคำสั่งซื้อ |
...
| UAT-C13 | ยืนยันรับสินค้า | ✅ ผ่าน | - | กดได้เฉพาะออเดอร์ของตนเองที่อยู่ระหว่างจัดส่ง |
...
จำนวน Test Case ทั้งหมด **26 รายการ**
...
จากการทดสอบ Test Case ผ่านการทดสอบ 26 จาก 26 รายการ ไม่พบปัญหาในการทดสอบ ฟังก์ชันสำคัญของระบบสามารถทำงานได้ตามความต้องการครบถ้วน 100%
